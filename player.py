...
class AsyncRequestModeView(AsyncBaseView):
    ...
    async def request_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try:
            ...
            await interaction.edit_original_response(
                content=f"❌ DM an den Gegner konnte nicht gesendet werden: {e}",
                view=self,
            )
            return
...
class OpponentConsentView(discord.ui.View):
    ...
    async def agree_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if channel is None or not isinstance(channel, discord.TextChannel):
            await interaction.edit_original_response(
                content="❌ Admin-Log-Channel wurde nicht gefunden.",
                view=None,
            )
            return
        ...
        await interaction.edit_original_response(
            content="✅ Du hast dem Async zugestimmt. Die Admins wurden informiert.",
            view=None,
        )
...
class SeedLinkModal(discord.ui.Modal, title="Seed setzen"):
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        try:
            ...
            await interaction.edit_original_response(
                content=f"{interaction.message.content}\n\n❌ Fehler beim Schreiben ins Async-Sheet: {e}",
                view=None,
            )
            return
        ...
        dm_text = (
            f"✅ Async wurde zugestimmt und ein Seed hinterlegt.\n"
            f"Spiel: {data['player1']} vs. {data['player2']}\n"
            f"Spielmodus: {data['selected_mode']}\n"
            f"Seed: {seed}\n"
